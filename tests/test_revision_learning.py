...
def test_revision_learning_separates_retained_removed_and_new_codes(tmp_path: Path) -> None:
    ...
    revision.write_text(
        ...
        .replace("parent_attempt_id: W-AD-2", "parent_attempt_id: W-AD-1")
    ...
    assert learning["comparisons"][0]["opportunity_confirmed_no_longer_observed_codes"] == ["LEX-WORDFORM"]
    assert learning["comparisons"][0]["not_comparable_without_opportunity_codes"] == []
...
def test_revision_learning_does_not_claim_improvement_without_a_recorded_opportunity(
    tmp_path: Path,
) -> None:
    write_attempt(tmp_path, "W-AD-1", "academic_discussion", "formal_original")
    write_attempt(tmp_path, "W-AD-1-R1", "academic_discussion", "revision")
    ...
    row = learning["comparisons"][0]
    assert row["opportunity_confirmed_no_longer_observed_codes"] == []
    assert row["not_comparable_without_opportunity_codes"] == ["GRAM-CLAUSE"]
